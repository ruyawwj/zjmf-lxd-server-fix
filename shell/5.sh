...
if [[ -n "$SWAPFILE_ACTIVE" ]]; then
    ...
    if [[ "$disable_swapfile" =~ ^[Yy]$ ]]; then
        echo "[INFO] 关闭 /swapfile swap ..."
        swapoff /swapfile
        echo "[INFO] /swapfile swap 已关闭。"
    else
        echo "[INFO] 保持 /swapfile swap 状态，不做修改。"
        exit 0
    fi
else
    echo "[INFO] /swapfile 当前未启用为 swap。"
fi
...
# 检查 fstab 是否已存在 swapfile 条目，避免重复写入
